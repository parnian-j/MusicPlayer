...
class HomePageState extends State<HomePage> {
  ...
  void dispose() {
    if (channel != null) {
      channel.sink.close();  // Close previous WebSocket connection
    }
    super.dispose();
  }
  ...
  Future<void> _loadAllSongs() async {
    ...
    channel = WebSocketChannel.connect(Uri.parse(widget.socketUrl));  // Open a new WebSocket connection
    ...
    try {
      ...
      if (stored != null) {
        ...
        _fetchServerSongs();  // Fetch songs from the server if not found in cache
      }
    } catch (e) {
  ...
  Future<void> _saveAllSongs() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final key = 'allSongs_${widget.username}'; // کلید اختصاصی یوزر
  ...
  Future<void> _loadPlaylists() async {
    try {
      ...
      stored ??= prefs.getString('playlists'); // برای سازگاری با نسخه قدیمی
      ...
      if (stored != null) {
        ...
        // اگر از کلید قدیمی خواندیم، به کلید جدید منتقل و قدیمی را پاک کن
  ...
  Future<void> _refreshPlaylists() async {
    ...
    // پلی‌لیست‌های دریافتی از سرور را بساز
    final fetched = (profile['playlists'] as List? ?? [])
        .map<Playlist>((e) => Playlist.fromJson(e as Map<String, dynamic>))
        .toList();

    setState(() {
      if (fetched.isNotEmpty) {
        playlists = fetched; // فقط اگر چیزی آمد، جایگزین کن
      }
      // (اگر برای songs هم همین مسئله را داری، مشابه همین نگه‌دار/مرج کن)
    });

    // هر چه شد، در نهایت لوکال را به‌روز کن تا بعدِ لاگینِ بعدی بماند
